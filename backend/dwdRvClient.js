...
// 0.1 mm/5min ≈ 1.2 mm/h, which is light drizzle detectable by the radar.
...
class DwdRvClient {
   ...
   * Optionally samples the precipitation value at a specific lat/lon in the same decode pass.
   *
   ...
   *   null if the member is missing; precipAtLocation is undefined when no target, null when outside coverage
   */
   ...
   *
   * When targetLat/targetLon are provided, also samples precipitation at that location
   * across the full 120-min nowcast horizon (independent of maxForecastFrames) to provide
   * an accurate short-term rain prediction.
   *
  ...
  async pollCycle(frameStore, pollingIntervalMinutes, maxForecastFrames = -1, targetLat = null, targetLon = null) {
    ...
    // Display leads: limited by maxForecastFrames
    ...
    // When a forecast location is set, also extract all 120-min leads for location sampling
    // (independent of maxForecastFrames, which only governs the display animation).
    ...
    // Track per-lead precipitation for location forecast
    ...
    // For leads beyond displayLeads (only when location forecast needs fuller horizon):
    // just decode + sample without rendering.
